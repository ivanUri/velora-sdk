...
async function apiFetch(url, init = {}) {
    const headers = { ...apiHeaders(init.apiKey), ...(init.headers ?? {}) };
...
async function downloadSnapshot(record, destDir, apiKey) {
    const res = await apiFetch(record.snapshot.files_url, { apiKey });
    if (!res.ok)
        throw new ProtocolError(`snapshot files list ${res.status}`);
    const listing = await res.json();
    mkdirSync(destDir, { recursive: true });
    for (const rel of listing.files) {
        const fileRes = await apiFetch(`${record.snapshot.base_url}${rel}`, { apiKey });
        if (!fileRes.ok)
            throw new ProtocolError(`snapshot download failed: ${rel} (${fileRes.status})`);
...
function applySessionToProfileDir(profileDir, session) {
...
async function downloadSession(record, profileDir, apiKey) {
    const res = await apiFetch(record.session.url, { apiKey });
    if (res.status === 404)
        return false;
    if (!res.ok)
        throw new ProtocolError(`session download ${res.status}`);
 ...
 */
export async function hydrateRemoteProfile(options) {
...
export async function flushRemoteSession(options) {
...
export function cleanupHydratedProfile(workDir) {
    if (existsSync(workDir))
        rmSync(workDir, { recursive: true, force: true });
}
